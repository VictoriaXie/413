...
class BertSelfAttention(nn.Module):
    ...
    def attention(self, key, query, value, attention_mask):
        ...
        # scale the scores by 1/sqrt(attention_head_size)
        scores = scores / math.sqrt(ahs)
        ...
        # transpose the attn_value back to [bs, seq_len, num_attention_heads, attention_head_size]
        attn_value = attn_value.transpose(1, 2).contiguous()
        # reshape the attn_value to [bs, seq_len, hidden_size]
        attn_value = attn_value.view(bs, seqlen, self.all_head_size)
        return attn_value
...
class BertLayer(nn.Module):
    ...
    def add_norm(self, input, output, dense_layer, dropout, ln_layer):
        ...
        """
        # first, we apply the dense layer and dropout to the output
    ...
    def forward(self, hidden_states, attention_mask):
        ...
        # another add-norm
        output = self.add_norm(attention_output, interm_output, self.out_dense, self.out_dropout, self.out_layer_norm)
        return output
